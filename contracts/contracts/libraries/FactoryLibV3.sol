...
library FactoryLibV3 {
  ...
  */
   ...
   * @param user : Caller.
   * @param token : ERC20 currency address to use as contribution base.
  ...
  */
  function _validateAllowance(
    ...
    view 
  {
    require(IERC20(token).allowance(user, address(this)) >= value, "FactoryLib: Insufficient allowance");
  }
  ...
  */   
    ...
    returns(C3.ReturnValue memory rv) 
  {
    ...
    Utils.assertTrue_2(cpp.duration > 0, cpp.duration <= 720, "Invalid duration"); // 720hrs = 30 days.
    _awardPoint(self.points, user, true);
    self.cSlots.increment();
    rv = _updatePoolSlot(self, cpp, router, _initializeUnit(self, cpp.unit));
    callback(self, rv);
    // _addUpToBank(bank, user, rv.rId);
    IERC20 token = self.pData.token;
    _validateAllowance(user, token, cpp.unit);
    _withdrawAllowanceToSafe(user, token, cpp.unit, safe);
  }
  ...
  /**@notice Send contribution to safe
   * @param user : User/Caller address
   * @param token: ERC20 token { USD contract address }
   * @param unitContribution: Unit contribution
   * @param safe : Address to hold funds on behalf of the members. The factory generates 
   * a safe by querying StrategyManager. Every address that create a pool must operate a 
   * safe. This process is managed internally for users.  
   */
  function _withdrawAllowanceToSafe(
    address user, 
    IERC20 token, 
    ...
    private 
  {
    require(IERC20(token).transferFrom(user, safe, unitContribution), "FactoryLib: Transfer failed");
  }
  ...
  function _initializePool(
    ...
    _p.uints = C3.Uints(cpp.quorum, 0, cpp.colCoverage, durInSec, cpp.intRate, cSlot, 0, _p.uints.userCount);
    _p.uint256s = C3.Uint256s(itr.fullInterest, itr.intPerSec, cpp.unit, cpp.unit, uId, rv.rId);
    _p.addrs = C3.Addresses(address(0), cpp.members[0]);
    _p.router = router;
  ...
  {
    address safe = _fetchAndValidateSafe(self);
    for(uint i = 0; i < cpp.members.length; i++) {
      address user = cpp.members[i];
      if(i == 0) {
        _p = _createPool(self, cpp, safe, user, C3.Router.PERMISSIONED, _callback).pool;
        _addNewContributor(self, _p.uints.cSlot, _p.uint256s.unit, user, true, true, true);
      } else {
        bool(user != _p.addrs.admin).assertTrue("Admin spotted twice");
        _addNewContributor(self, _p.uints.cSlot, _p.uint256s.unit, user, false, true, false);
    ...
    returns (C3.Pool memory _p)
  {
    ...
    _p = _createPool(self, cpp, _fetchAndValidateSafe(self), user, C3.Router.PERMISSIONLESS, _callback).pool;
    _addNewContributor(self, _p.uints.cSlot, _p.uint256s.unit, user, true, true, true);
  }
  ...
  */
    ...
    ) 
  {
    ...
    if(_ab.isPermissioned) {
      ...
      self.cData[_p.uints.cSlot][_getSlot(self.slots, caller, _ab.unit).value].sentQuota = true;
    } else {
      if(_p.uints.userCount >= _p.uints.quorum) revert PoolIsFilled();
      _mustNotBeAMember(self, _ab.unit, caller);
      _addNewContributor(self, _p.uints.cSlot, _ab.unit, caller, false, true, true);
      _p = _getCurrentPool(self, _ab.unit).data;
    }
    ...
    IERC20 token = self.pData.token;
    _validateAllowance(caller, token, _p.uint256s.unit);
    _withdrawAllowanceToSafe(caller, token, _p.uint256s.unit, self.pData.safe);
  ...
  */  
    ...
    returns (C3.Pool memory _p) 
  {
    ...
    C3.Contributor memory cData = C3.Contributor({
      ...
      // loan: IBank(arg.pool.addrs.bank).borrow{value: arg.msgValue}( caller, arg.pool.addrs.asset, arg.pool.uint256s.currentPool, arg.fee, computedCollateral, arg.pool.uint256s.rId), // Create transaction in safe
    ...
    arg.pool.uint256s.currentPool = 0;
    _p = arg.pool;
  }
  ...
  */
    ...
    returns(uint amtPayBackInUSD, uint colWithdrawn, C3.Pool memory _p)
  {
    ...
    IERC20 token = self.pData.token;
    _validateAllowance(pb.user, token, debt);
    _withdrawAllowanceToSafe(pb.user, token, debt, self.pData.safe);
    ...
    returns (C3.Contributor memory _liq, bool defaulted, uint currentDebt, C3.Slot memory slot, address defaulter) 
  {
    ...
    (_liq, defaulted, currentDebt, slot, defaulter)
      = 
        _now() <= prof.payDate? 
          (_liq, false, uint256(0), C3.Slot(0, false, false), address(0)) 
            : 
              ( prof, true, _getCurrentDebt( self, unit, prof.id).debt, self.slots[prof.id][unit], prof.id);
  }
  ...
  */
    ...
    returns(C3.Balances memory balances) 
  {
    ...
    address safe = self.pData.safe;
    IERC20 token = self.pData.token;
    return C3.Balances({
      xfi: safe.balance,
      erc20: IERC20(token).balanceOf(safe)
    });
  }

  /** @dev Update Contract variables
    * @param self : Storage. 
    * @param safe : Safe wallet.
    * @param token : Token contract interface.
  ...
  function setContractData(
    ...
    returns(bool)
  {
    if(address(token) != address(0)) self.pData.token = token;
    if(safe != address(0)) self.pData.safe = safe;
